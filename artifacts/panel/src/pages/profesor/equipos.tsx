...
import {
  useListTeacherMyTeams,
  useCreateTeacherMyTeam,
  useGetTeacherMyTeam,
  useDeleteTeacherMyTeam,
  useAddTeacherMyTeamMember,
...
function TeamDetail({ teamId, onClose }: { teamId: number; onClose: () => void }) {
  const { data: team, isLoading } = useGetTeacherMyTeam(teamId);
  const { data: students } = useListTeacherMyStudents({ groupId: team?.groupId });
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>{team?.groupName}</DialogDescription>
        </DialogHeader>
...
export default function ProfesorEquipos() {
  const { data: teams, isLoading } = useListTeacherMyTeams();
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: getListTeacherMyTeamsQueryKey() });
  const createTeam = useCreateTeacherMyTeam({ mutation: { onSuccess: () => { invalidate(); setIsCreateOpen(false); setForm({ groupId: 0, name: "" }); toast({ title: "Equipo creado" }); }, onError: (err: any) => toast({ variant: "destructive", title: "Error", description: err?.data?.error ?? "" }) } });
  const deleteTeam = useDeleteTeacherMyTeam({ mutation: { onSuccess: () => { invalidate(); setTeamToDelete(null); toast({ title: "Equipo eliminado" }); } } });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Equipos colaborativos: un Dolibarr compartido por equipo.</p>
        </div>
